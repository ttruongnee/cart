document.getElementById('main-form').addEventListener('submit', async function (event) {
    ...
    const url = "https://localhost:44366/api-nguoidung/ControllersTaiKhoan/DangNhapUser";
    try {
...
async function getGioHangByUser(maGioHang) {
    const url = `https://localhost:44366/api-nguoidung/ControllersGioHang/GetGioHangByUser/${maGioHang}`;
    try {
        const response = await fetch(url);
...
async function GetGioHangFromAPI() {
    ...
    for (let item of gioHang) {
        for (let ct of item.listjson_chitiet) {
            ...
            try {
                const truyenInfo = await fetch(`https://localhost:44366/api-nguoidung/ControllersTruyen/SearchByID/${maTruyen}`).then(response => response.json());
